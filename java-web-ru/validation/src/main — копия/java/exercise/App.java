...
public final class App {
    ...
    public static Javalin getApp() {
        ...
        // BEGIN
        app.get("/articles/build", ctx -> ctx.render("articles/build.jte"));
        ...
        app.post("/articles", ctx -> {
            ...
            try {
                ctx.formParamAsClass("title", String.class)
                        ...
                        .check(value -> ArticleRepository.getEntities().stream().noneMatch(article -> article.getTitle().equals(value)),
                                "Статья с таким названием уже существует").get();
                ctx.formParamAsClass("content", String.class)
                        .check(value -> value.length() > 10,
                        "Статья должна быть не короче 10 символов").get();
